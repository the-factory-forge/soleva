...
test("background video respects motion preferences and keyboard playback controls", async ({
  page,
}) => {
  ...
  await expect
    .poll(() =>
      video.evaluate((element) => (element instanceof HTMLVideoElement ? element.currentTime : 0)),
    )
    .toBeGreaterThan(0);
  ...
  await page
    .getByRole("button", {
      name: `${fr.home.hero.play_video} : Soleva - L’aventure Soleva`,
      exact: true,
    })
    .click();
  await expect(page.getByTitle("Soleva - L’aventure Soleva")).toHaveAttribute(
    "src",
    /youtube-nocookie.com\/embed\/6ScnYhFPv5w\?autoplay=1/,
  );
});

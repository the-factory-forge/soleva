...
- When navigation-critical route logic needs auth data, read `authQueryOptions` through the existing `context.queryClient`, following `_auth/$lang/app/route.tsx`.

## Route Guards

- The `src/routes/_auth/route.tsx` layout redirects to `/` when the optional auth feature is disabled.
- Protected app layout is `src/routes/_auth/$lang/app/route.tsx`.
  - It enforces auth in `beforeLoad` using TanStack Query and preserves the locale in redirects.
- Localized login/signup routes under `src/routes/_auth/$lang/` redirect authenticated users to their localized dashboard.
